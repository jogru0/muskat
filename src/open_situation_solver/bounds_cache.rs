use crate::{
    bounds::Bounds, game_type::GameType,
    open_situation_solver::bounds_and_preference::BoundsAndMaybePreference, role::Role,
    situation::OpenSituation, trick_yield::TrickYield,
};
...
pub fn open_situation_reachable_from_to_u32_key(
    ...
    let first_defener_card = unsafe { cellar.remove_smallest_unchecked() };
    let second_defener_card = unsafe { cellar.remove_smallest_unchecked() };
...
{
    pub fn new(open_situation_to_key: OpenSituationToKey, game_type: GameType) -> Self {
    ...
    OpenSituationToKey: Fn(OpenSituation) -> Key,
{
    ...
    fn update_existing(
        ...
        // {
        //     let lower_bound = open_situation.lower_bound_non_null_declarer_forehand(trump);
