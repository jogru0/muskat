	...
	public:
		void cheat(const Situation &situation) override {
			...
			say(
				...
				std::to_string(static_cast<size_t>(worst_case_score_from_here.points())) +
				'.'
			);
		...
		virtual void inform_about_move(Card card) final {
			...
			m_points_declarer.add(m_current_situation.play_card(card, m_game));
		}
		...
		virtual auto request_move() -> Card final {
			...
			say(
				...
				to_string(card) +
				" to force score " +
				std::to_string(static_cast<size_t>(m_points_declarer.points() + worst_case_score_from_here.points())) +
				end_of_sentence
			);
